...
const importedDaySchema = importedPlanSchema.properties.days.items;
importedExerciseSchema.properties.exerciseId = {
  anyOf: [{ type: "string" }, { type: "null" }],
};
importedExerciseSchema.properties.sourceName = { type: "string" };
importedExerciseSchema.properties.programmingRole = {
  anyOf: [
    { type: "string", enum: ["main", "accessory", "power"] },
    { type: "null" },
  ],
};
...
importedDaySchema.properties.warmup = {
  anyOf: [
    {
      type: "object",
      additionalProperties: false,
        ...
        items: {
          ...
          items: {
            ...
            properties: {
              ...
              seconds: { anyOf: [{ type: "integer" }, { type: "null" }] },
              minutes: { anyOf: [{ type: "number" }, { type: "null" }] },
              notes: { anyOf: [{ type: "string" }, { type: "null" }] },
            },
            required: ["label", "sets", "reps", "seconds", "minutes", "notes"],
          },
        },
      },
      required: ["items"],
...
importedDaySchema.required.push("warmup");
schemas["import-plan"] = {
...
instructions["import-plan"] =
  ...
  instructions["import-plan"];
instructions["import-plan"] =
  "CRITICAL STRUCTURE RULE: a bare ordinal such as '1.', '2.', or '7.' followed by an exercise name is only an exercise number. It never starts a day or workout. Start a new days item only from an explicit weekday, date, or wording such as Day, Dan, Workout, Trening, or Session. If none of those explicit boundaries exists, the response must contain exactly one days item holding every primary numbered exercise. Numbered exercise notes may span several following lines. Merge consecutive working-set lines into that numbered exercise in source order: for example, one set at one load followed by two sets at another load is one three-working-set exercise with per-set loads. Tolerate ordinary spelling mistakes in localized set and rep words. A line explicitly labelled warm-up or 'ogrevalni set' is not a working set: never include its load in weightKg or setWeightsKg and never increase the exercise's working sets count because of it. Preserve it as warmup content or an exact note tied to the exercise context. A standalone alternative marker such as 'ali', 'or', 'oder', 'o', or 'ou', followed by the same exercise-slot number, introduces a substitute for that slot; it is not a new workout, day, or additional scheduled exercise. The first listed choice is the scheduled exercise. Since the response has no alternatives field, append each substitute's exact exercise name and exact written prescription to the scheduled exercise notes, clearly prefixed as an alternative, and do not return the substitute as another exercise. Text such as 'x reps' means the repetition count is explicitly unspecified: never guess it. When another set line for that same scheduled exercise has a numeric rep target, use only that known numeric target in repMin and repMax and preserve the unknown set verbatim in notes. Never invent repetitions for an alternative that is retained only in notes. " +
  instructions["import-plan"];
instructions.coach =
...
const fewerHardPrescriptionInstruction =
  " For effortPolicy mode fewer-hard, ROOK's deterministic prescription is authoritative and overrides every generic rep-range guideline: return exactly 2 working sets. Set programmingRole to power for power exercises, main for the session's primary strength anchor or anchors, and accessory for supporting work; a full-body template may have one lower-body and one upper-body main anchor. Build muscle uses 6-8 for every compound and 8-12 for isolations. General fitness and Lose fat use 6-8 for a main compound, 8-10 for accessory compounds, and 8-12 for isolations. Get stronger uses 3-6 for a main compound, 6-8 for accessory compounds, and 8-12 for isolations. Athletic performance uses 2-5 for power exercises, 3-6 for a main strength compound, 6-8 for accessory compounds, and 8-12 for isolations. Power exercises always use 2-5 and stop on speed or technical-quality loss, not an RIR target; set the legacy response field targetRir to 0 only as a transport sentinel, which the application discards. Timed exercises use their exact catalog durationRange. Never return 10-15, 8-20, or any other invented range for fewer-hard. If previousValidationError reports an effort rep range, correct it exactly rather than widening a range.";
instructions.plan += fewerHardPrescriptionInstruction;
instructions.plan +=
...
instructions["plan-repair"] += fewerHardPrescriptionInstruction;
