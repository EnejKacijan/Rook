...
it('ships all 26 reviewed masters unchanged, with no embedded bitmap or schematic stroke',()=>{expect(files).toHaveLength(26);for(const file of files){const s=readFileSync(new URL(file,masters),'utf8');expect(readFileSync(new URL(file,art),'utf8')).toBe(s);expect(s).toMatch(/width="512" height="512"/);expect(s).toContain('fill="#1f6b4c"');expect(s).not.toMatch(/<image|<filter|stroke=|<text/);}});
