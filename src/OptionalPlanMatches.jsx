...
export function OptionalPlanMatches({program,profile,onMatch,onOriginal,onBack,onRemove,onUndo}){
  ...
  const liveEntries=group.entries.filter(entry=>program.days.find(d=>d.id===entry.dayId)?.exercises.some(e=>e.id===entry.exercise.id));
  const candidate=liveEntries.length>0&&group.available.find(item=>item.id===(group.match.exerciseId||group.match.suggestedExerciseId));
  const select=(id,event)=>{
    ...
    for(const entry of group.entries.filter(e=>program.days.find(d=>d.id===e.dayId)?.exercises.some(x=>x.id===e.exercise.id)))id?onMatch(entry.dayId,entry.exercise.id,id):onOriginal(entry.dayId,entry.exercise.id);
  ...
  return <section ref={root} className="import-resolution" data-import-step-back="true" aria-label="Optional exercise matches">
    ...
    <div ref={scroll} className="import-decision-scroll" data-swipe-back-content><div className="import-decision-content">
      ...
      <div className="import-group-source"><blockquote>{group.source}</blockquote></div>
      {onRemove&&group.entries.map(entry=>{const present=program.days.find(d=>d.id===entry.dayId)?.exercises.some(e=>e.id===entry.exercise.id);return <div key={entry.exercise.id}>{group.entries.length>1&&<small>{program.days.find(d=>d.id===entry.dayId)?.name} · occurrence {group.entries.indexOf(entry)+1}</small>}{present?<button className="text-button" onClick={()=>onRemove(entry.dayId,entry.exercise.id)}>Remove from plan</button>:<p role="status">Removed from plan · Source preserved <button className="text-button" onClick={()=>onUndo?.(entry.exercise.id)}>Undo</button></p>}</div>;})}
      ...
      {liveEntries.length>0&&<button className="button quiet" onClick={()=>{setBrowse(true);requestAnimationFrame(()=>input.current?.focus());}}>CHOOSE ANOTHER</button>}
      {browse&&liveEntries.length>0&&<><input ref={input} className="text-answer" type="search" aria-label="Search exercise matches" value={query} onChange={event=>setQuery(event.target.value)}/><div className="import-resolution-options">{choices.map(item=><button className="button secondary" key={item.id} onClick={event=>select(item.id,event)}>{item.name}</button>)}</div></>}
    </div></div><SheetActionFooter separate importViewport><button className="button secondary" onClick={event=>select(null,event)}>{liveEntries.length?'KEEP ORIGINAL':'CONTINUE'}</button></SheetActionFooter>
  </section>;
}
