...
import {workoutSetSummary,workoutPerformedDate} from './domain.js';
...
export function completionRecognition(session,history=[],{eligible=()=>false}={}){
 ...
 const prior=[...new Map(history.filter(w=>w.id!==session.id&&w.completedAt&&new Date(w.completedAt)<=new Date(session.completedAt)&&workoutPerformedDate(w)<=workoutPerformedDate(session)).map(w=>[w.id,w])).values()];
