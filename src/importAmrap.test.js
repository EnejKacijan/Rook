...
it('preserves hold seconds and accepts inline rounds without inventing reps',async()=>{const {e}=await make('Plank 3x45 sec hold');expect(e.repMin).toBe(45);expect(e.measure).toBe('seconds');const r=await make('Y Balance Reach 2 kroga');expect(r.r.sourceReview.issues.some(i=>i.field==='prescription')).toBe(false);expect(r.e.importedRoundPrescription).toEqual({count:2});expect(r.e.sets).toHaveLength(2);expect(r.e.sets.every(s=>s.reps===null)).toBe(true);});
