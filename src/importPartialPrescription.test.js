...
it.each([
 ['Cable Row 3 sets',3,null,['reps'],null],
 ['Cable Row 10 reps',0,10,['sets'],null],
 ['Bench Press 60kg',0,null,['sets','reps'],60],
 ['Cable Row 3 sets RIR 2',3,null,['reps'],null],
 ['Cable Row 3x',3,null,['reps'],null],
 ['Cable Row',0,null,['sets','reps'],null],
 ...
 expect(e.sets).toHaveLength(count);expect(e.repMin).toBe(reps);expect(e.partialPrescription.missing).toEqual(missing);expect(e.partialPrescription.weight).toBe(weight);
 ...
 expect(resolvePartialPrescription(e,{sets:3,repMin:10,repMax:10})).toBe(true);
...
it('case 26 preserves all work and known sets through resolution',async()=>{
 ...
 expect(r.sourceReview.issues.filter(i=>i.field==='prescription')).toHaveLength(2);
 const e=r.program.days[0].exercises[1];resolvePartialPrescription(e,{sets:9,repMin:10,repMax:10});expect(e.sets).toHaveLength(3);expect(e.sets.every(s=>s.reps===10)).toBe(true);
});
it('invalid resolution leaves the source draft intact',async()=>{
 const r=await parse('Cable Row 3 sets'),e=r.program.days[0].exercises[0],before=JSON.stringify(e);
