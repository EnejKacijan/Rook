...
import { displayWeight, saveState } from './domain.js';
...
export function BlockReviewSheet({state,update,close,Header,blockId,onBack}) {
  ...
  const candidates=source?nextBlockReplacementChoices(state,source).filter(item=>item.name.toLowerCase().includes(query.trim().toLowerCase())):[];
  return <main ref={ref} className="screen detail-screen block-review-sheet">
    <Header title={step==='next'?'Next block':step==='replacement'?'Choose replacement':'Block review'} onClose={close} onBack={step!=='summary'||onBack?back:undefined}/>
    {step==='summary' && <>
    ...
    {step==='next' && proposal && <>
      ...
      <div className="flexible-week-footer">
        {active && <p role="status">Finish the active workout before starting the next block.</p>}
        {error && <p role="alert">{error}</p>}
        {error && !error.includes('couldn’t save') && <button className="text-button" onClick={()=>prepare(proposal.repeat)}>REVIEW AGAIN</button>}
        <button className="button primary" disabled={active || !!error && !error.includes('couldn’t save')} onClick={apply}>START NEXT BLOCK</button><button className="button quiet" onClick={close}>CANCEL</button></div>
    </>}
