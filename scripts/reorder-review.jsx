...
const params=new URLSearchParams(location.search),scenario=params.get('case')||'active',key='rook-reorder-review:'+scenario;
...
qa.innerHTML='<summary>Reorder QA</summary><label>Theme <select id="theme"><option>premium-dark</option><option>premium-light</option><option>dark</option><option>light</option></select></label><label>From <input id="from" type="number" value="0" min="0"></label><label>To <input id="to" type="number" value="2" min="0"></label><button id="measure">Measure</button><button id="drag">Drag</button><button id="hold">Hold drag</button><button id="release">Release</button><button id="cancel">Cancel drag</button><button id="swipe">Swipe source</button><button id="edge">Autoscroll edge</button><button id="middle">Leave edge</button><pre id="proof"></pre>';
...
const handles=()=>[...document.querySelectorAll('#root .rook-reorder-handle')].filter(h=>h.dataset.reorderKind!=='workout'&&h.getBoundingClientRect().height>0);
const theme=()=>{const value=el('theme').value;Object.assign(document.documentElement.dataset,{theme:value.startsWith('premium')?'premium':value,style:value.startsWith('premium')?'premium':'standard',appearance:value.endsWith('dark')?'dark':'light'});};el('theme').onchange=theme;theme();
const rect=e=>e.getBoundingClientRect();let held=null;
function proof(){
 const list=document.querySelector('.up-next-queue'),items=[...document.querySelectorAll('#root .rook-reorder-handle')].filter(h=>rect(h).height>0);
 return {width:innerWidth,theme:el('theme').value,overflow:document.documentElement.scrollWidth>innerWidth,
...
async function drag(hold=false){const source=handles()[Number(el('from').value)],target=handles()[Number(el('to').value)];if(!source||!target)throw Error('Choose visible handle indices');source.scrollIntoView({block:'center'});await frame();await frame();const b=rect(source),t=rect(target),x=b.left+b.width/2,y=b.top+b.height/2,dy=t.top+t.height/2-y+(t.top>b.top?10:-10);held={target:source,x,y:y+dy};send('touchstart',source,x,y);for(let i=1;i<=8;i++){send('touchmove',source,x,y+dy*i/8);await frame();}if(!hold){send('touchend',source,x,y+dy);held=null;}await settle();}
async function run(kind){qa.open=false;el('proof').textContent='Running';
 ...
 if(kind==='swipe'){const h=handles()[Number(el('from').value)],r=h.closest('[data-swipe-row]');r.scrollIntoView({block:'center'});await frame();await frame();const target=r.querySelector('.up-next-main,.plan-editor-heading'),b=rect(r),x=b.left+b.width*.65,y=b.top+b.height/2;send('touchstart',target,x,y);for(let i=1;i<=8;i++){send('touchmove',target,x-i*12,y);await frame();}send('touchend',target,x-96,y);await settle();}
