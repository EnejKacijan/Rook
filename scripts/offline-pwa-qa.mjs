...
const sessionId = await page.evaluate(() => JSON.parse(localStorage.getItem('lift-v2-state')).activeWorkout.id); const weightInput = page.getByRole('textbox', { name: /Weight in kg for set 1/ }); if (await weightInput.count()) { await weightInput.fill('40'); await weightInput.press('Enter'); } const expectedWeight = await page.evaluate(() => JSON.parse(localStorage.getItem('lift-v2-state')).activeWorkout.exercises[0].sets[0].weight); await page.getByRole('button', { name: 'Log set 1' }).click(); await page.reload({ waitUntil: 'domcontentloaded' }); await page.getByRole('button', { name: 'RESUME WORKOUT' }).click();
...
assert.equal(await page.locator('.exercise-heading-art').evaluateAll(images=>images.filter(image=>!image.complete||!image.naturalWidth).length), 0, 'Offline artwork is either cached and valid or omitted; never broken');
