...
await page.route("**/api/ai/status", (route) =>
  route.fulfill({
    ...
    body: JSON.stringify({ available: true, provider: "local-test" }),
  }),
);
await page.route("**/api/ai", async (route) => {
  ...
  const data =
    body.operation === "follow-ups"
      ? { questions: [] }
      : body.operation === "plan"
        ? authoredPlan
        : { text: "Test reply.", action: null };
  await route.fulfill({
    status: 200,
    contentType: "application/json",
...
assert.equal(
  await page.getByRole("button", { name: "BUILD MY PLAN" }).count(),
  1,
  "landing leads with personalized plan creation",
);
assert.equal(
  await page.getByRole("button", { name: /Already have a plan/i }).count(),
  1,
  "landing exposes existing-plan import",
);
await page.getByRole("button", { name: /Already have a plan/i }).click();
assert.equal(
  await page
    .getByRole("heading", { name: "Bring your existing workout into Rook." })
    .count(),
  1,
);
...
assert.equal(
  await beginnerOption
    .getByText("New to structured training", { exact: true })
    .count(),
  1,
  "experience option includes concise guidance",
);
const beginnerBox = await beginnerOption.boundingBox();
assert.ok(
...
assert.equal(
  await page.getByText("Full gym access", { exact: true }).count(),
  1,
  "commercial selection confirms implicit equipment",
);
...
assert.equal(
  ...
  true,
  "home setup requires at least one equipment choice",
);
...
assert.equal(
  ...
  "true",
  "a third choice is unavailable once two priorities are selected",
);
assert.equal(
  await priorityArea.getByText("2 of 2 selected", { exact: true }).count(),
  1,
  "the active priority limit is visible",
);
assert.equal(
  await priorityArea.getByText("Deselect one to choose another.", { exact: true }).count(),
  1,
  "the priority-limit recovery instruction is visible",
);
await page.getByRole("button", { name: "CONTINUE" }).click();
assert.equal(
  await page
    .getByRole("button", { name: /Balanced starting point/ })
...
await page
  ...
  .fill(testProfile.trainingPreferences);
await page.getByRole("button", { name: "BUILD MY PLAN" }).click();
...
assert.equal(
  ...
  false,
  "generated onboarding plan remains a preview before confirmation",
);
...
assert.equal(
  planRequests.length,
  0,
  "onboarding uses the deterministic personalized generator without external data transfer",
);
assert.equal(
...
assert.deepEqual(
  ...
  ["full gym"],
  "Commercial setup supplies implicit full-gym access",
);
assert.deepEqual(
  saved.profile.prioritySources.manual,
  ["Shoulders", "Chest"],
  "the locked third choice does not replace either selected priority",
);
assert.equal(saved.ai.lastPlanSource, "personalized-template");
assert.equal(
  saved.program.templateId,
  fixture.templateId,
  "the selected split preference produces the expected stable template",
);
assert.deepEqual(
  saved.program.days.map((day) => day.name),
  fixture.days.map((day) => day.name),
  "the local generator preserves the preference-shaped session structure",
);
assert.equal(
  validateProgram(saved.program, saved.profile, { requireProgramQuality: true })
    .valid,
  true,
  "the accepted local plan passes the full quality contract",
);
await page.reload({ waitUntil: "networkidle" });
const reloaded = await page.evaluate(() =>
  JSON.parse(localStorage.getItem("lift-v2-state")),
);
assert.equal(reloaded.profile.environment, "Commercial gym");
assert.deepEqual(
  reloaded.profile.equipment,
  ["full gym"],
  "conditional equipment semantics persist after reload",
);
const importContext = await browser.newContext({
  viewport: { width: 390, height: 844 },
});
const importPage = await importContext.newPage();
const importedFixture = structuredClone(authoredPlan);
importedFixture.days.forEach((day) =>
  day.exercises.forEach((exercise) => {
    exercise.sourceName = exerciseCatalog[exercise.exerciseId].name;
...
await importPage.route("**/api/ai", async (route) => {
  const body = route.request().postDataJSON();
  const data =
    body.operation === "import-plan"
      ? importedFixture
      : body.operation === "follow-ups"
        ? { questions: [] }
        : { text: "Test reply.", action: null };
...
const importSourceText = importedFixture.days
  .flatMap((day) => [
    `${day.weekday} · ${day.name}`,
    ...day.exercises.map(
      (exercise) =>
        `${exercise.sourceName} — ${exercise.sets} × ${exercise.repMin}–${exercise.repMax}`,
    ),
  ])
  .join("\n");
await importPage.goto("http://127.0.0.1:4173", { waitUntil: "networkidle" });
...
assert.equal(
  await importPage.getByText("Check exercise name", { exact: true }).count(),
  0,
  "an exact custom source name does not use obsolete error wording",
);
assert.equal(
  await importPage.getByText(/0 RIR/).count(),
  0,
  "missing RIR is not invented in preview",
);
assert.equal(
...
let imported = await importPage.evaluate(() =>
  JSON.parse(localStorage.getItem("lift-v2-state")),
);
...
assert.equal(imported.profile.environment, "Commercial gym");
assert.deepEqual(imported.profile.equipment, ["full gym"]);
...
assert.equal(
  ...
  "ai-import",
  "landing import persists after reload",
);
assert.equal(
  imported.program.days[0].exercises[0].importedName,
  "Rotating Iso Cable Sweep",
...
await browser.close();
console.log(
  "AI planner QA passed: landing routes, personalized plan preservation, conditional equipment, Notes import, and persistence verified without external data transfer.",
);
