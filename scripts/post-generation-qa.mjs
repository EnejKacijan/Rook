...
async function buildFirstPlan(page, days, priorities = ['Balanced']) {
  ...
  await page.getByRole('button', { name: 'Build muscle' }).click(); await page.getByRole('button', { name: /^Beginner/ }).click();
...

{
  ...
  await page.getByRole('button', { name: 'PROFILE', exact: true }).click(); assert.equal(await page.getByText(/2 days\/week/, { exact: false }).count(), 1); await openProfileArea(page, 'training'); await page.getByRole('button', { name: /^Training priorities/ }).click(); assert.equal(await page.getByRole('button',{name:'Chest',exact:true}).getAttribute('aria-pressed'),'true'); assert.equal(await page.getByRole('button',{name:'Back',exact:true}).getAttribute('aria-pressed'),'true'); assert.equal(await page.getByText('2 of 2 selected', { exact: true }).count(), 1); assert.equal(await page.getByText('Deselect one to choose another.', { exact: true }).count(), 1); await page.screenshot({ path: output('390-profile-after-generation.png'), fullPage: false }); assert.deepEqual(errors, []); await context.close();
}
