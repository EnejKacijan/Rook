...
await page.getByRole('spinbutton', { name: /Weight in kg for set 1/ }).fill('30'); await page.getByRole('button', { name: 'Log set 1' }).click();

await page.getByRole('button', { name: 'Back to Today' }).click(); await page.getByRole('button', { name: 'COACH' }).click(); await page.getByLabel('Ask Coach').fill('Adapt today to 35 minutes.'); await page.getByRole('button', { name: 'Send message' }).click(); await page.getByRole('button', { name: 'APPLY TO TODAY' }).click(); await page.getByRole('status').filter({ hasText: 'Today updated' }).waitFor();
await page.getByRole('button', { name: 'View workout →' }).click(); await page.getByRole('button', { name: 'RESUME WORKOUT' }).click();

await context.setOffline(true); await page.screenshot({ path: output('390-offline-active.png'), fullPage: false }); await page.getByRole('button', { name: 'Back to Today' }).click(); await page.getByRole('button', { name: 'COACH' }).click(); await page.getByText('AI Coach is unavailable. Logging and data-based progression still work locally.', { exact: true }).waitFor(); await page.screenshot({ path: output('390-coach-offline.png'), fullPage: false }); await page.getByRole('button', { name: 'TODAY', exact: true }).click(); await page.getByRole('button', { name: 'RESUME WORKOUT' }).click(); const activeButtons = await page.locator('button.check:not([disabled])[aria-label^="Log set"]').count(); if (activeButtons) await page.locator('button.check:not([disabled])[aria-label^="Log set"]').first().click();
