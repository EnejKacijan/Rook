...
assert.equal(await page.getByRole('button', { name: 'SHARE WORKOUT' }).count(), 0, 'Today keeps the primary workout action focused and does not expose per-session export');
...
assert.equal(await page.getByText('WORKOUT EXERCISES', { exact: true }).count(), 1, 'the active-workout snapshot keeps its established workout-context heading');
