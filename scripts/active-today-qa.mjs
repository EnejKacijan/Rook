...
assert.match(await page.locator('.active-workout-hero > p').textContent(), new RegExp(`^0 of ${totalSets} sets · 18 min$`));
assert.equal(await page.getByRole('button', { name: 'RESUME WORKOUT' }).count(), 1);
assert.equal(await page.locator('.exercise-preview .exercise-row-main img').count() > 0, true, 'active-date rows honor enabled exercise illustrations');
...
assert.equal(await page.locator('.exercise-preview .exercise-row-main img').count() > 0, true, 'another workout day keeps its normal illustrated exercise list');
...
assert.match(await page.locator('.active-workout-hero > p').textContent(), new RegExp(`^2 of ${totalSets} sets · 18 min$`));
assert.equal(await page.getByRole('button', { name: 'START WORKOUT' }).count(), 0);
