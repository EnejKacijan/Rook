...
const page=await context.newPage();await page.route('**/api/**',r=>r.fulfill({json:{available:false}}));await page.goto('http://127.0.0.1:4175'); await openFirstRunLanding(page);await page.getByRole('button',{name:'BUILD MY PLAN',exact:true}).click();
...
await page.getByRole('button',{name:'Back to plan options',exact:true}).click();await page.getByRole('button',{name:'BUILD MY PLAN',exact:true}).waitFor();await page.getByRole('button',{name:'BUILD MY PLAN',exact:true}).click();assert.equal(await page.getByRole('button',{name:'lb',exact:true}).getAttribute('aria-pressed'),'true');assert.equal(await next.isEnabled(),true,'Back/reopen preserves the entered age as well as units');assert.match(await page.getByRole('combobox',{name:'Age range'}).textContent(),/18–29/);
