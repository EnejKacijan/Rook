import {openFirstRunLanding,chooseOnboardingAnswer,waitForOnboardingStep} from './qa-current-navigation.mjs';
import assert from 'node:assert/strict';
import {mkdir} from 'node:fs/promises';
import {chromium} from 'playwright-core';
import {blankState} from '../src/domain.js';
const out='artifacts/onboarding-weekly-structure';await mkdir(out,{recursive:true});
...
const page=await context.newPage();await page.route('**/api/**',r=>r.fulfill({json:{available:false}}));await page.goto('http://127.0.0.1:4175'); await openFirstRunLanding(page);await page.getByRole('button',{name:'BUILD MY PLAN',exact:true}).click();await page.getByRole('combobox',{name:'Age range'}).click();await page.getByRole('option',{name:'18–29',exact:true}).click();await page.getByRole('button',{name:'CONTINUE',exact:true}).click();
const step=async n=>{await waitForOnboardingStep(page,n);assert.equal(await page.locator('.step-count').textContent(),`STEP ${n}/8`);};
...
await page.getByRole('button',{name:goal,exact:true}).evaluate(e=>{e.dispatchEvent(new MouseEvent('click',{bubbles:true,detail:1}));e.dispatchEvent(new MouseEvent('click',{bubbles:true,detail:2}));});await step(3);await page.waitForTimeout(350);
...
await chooseOnboardingAnswer(page,'Build muscle',3);await shot('experience');
for(const experience of ['Beginner','Intermediate','Advanced']){
const button=page.getByRole('button',{name:new RegExp(`^${experience}`)});await button.evaluate(e=>{e.click();e.click();});await step(4);await page.waitForTimeout(350);
...
await page.getByRole('button',{name:/^Beginner/}).evaluate(e=>e.dispatchEvent(new MouseEvent('click',{bubbles:true,detail:2})));await step(3);await page.waitForTimeout(350);
await chooseOnboardingAnswer(page,/^Beginner/,4);await shot('schedule');await page.getByRole('button',{name:'3 days',exact:true}).click();await page.getByLabel('Any day works').check();await page.getByRole('button',{name:'60 min',exact:true}).click();await page.getByRole('button',{name:'CONTINUE',exact:true}).click();
...
await page.getByRole('button',{name:'CONTINUE',exact:true}).click();await step(8);
assert.equal(await page.getByText('FINAL PREFERENCES',{exact:true}).count(),1);
assert.equal(await page.getByText('WEEKLY STRUCTURE',{exact:true}).count(),1);
const auto=page.getByRole('button',{name:/^LET ROOK CHOOSE/});
assert.equal(await auto.getAttribute('aria-pressed'),'true');
const disclosure=page.getByRole('button',{name:'I already have a preferred weekly structure'});
assert.equal(await disclosure.getAttribute('aria-expanded'),'false');
await shot('step8-default');
await disclosure.click();assert.equal(await disclosure.getAttribute('aria-expanded'),'true');
const manual=page.locator('#specific-split-options button').first();
const manualName=await manual.innerText();await manual.click();
assert.equal(await auto.getAttribute('aria-pressed'),'false');
await page.getByRole('button',{name:'Back',exact:true}).click();await step(7);
await page.getByRole('button',{name:'CONTINUE',exact:true}).click();await step(8);
assert.equal(await auto.getAttribute('aria-pressed'),'false');
...
assert.equal(await page.getByRole('textbox',{name:'Restrictions or clinician limits'}).isVisible(),true);
assert.equal(await page.getByRole('button',{name:'BUILD MY PLAN',exact:true}).count(),1);
