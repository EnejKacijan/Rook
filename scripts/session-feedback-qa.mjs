...
let restoreState;
try{
 for(const width of process.env.ROOK_FEEDBACK_BLOCK_ONLY?[]:[320,390,430])for(const appearance of ['light','dark'])for(const style of ['standard','premium']){
  const {page,context,errors}=await open(width,appearance,style),key=`${width}-${style}-${appearance}`;
  await shot(page,`${key}-completion-top`);await page.locator('.session-feedback').scrollIntoViewIfNeeded();await shot(page,`${key}-unset`);
  ...
  await page.getByRole('button',{name:'Skip',exact:true}).click();assert.equal((await saved(page)).workouts.at(-1).sessionFeedback,'skipped');await shot(page,`${key}-skipped`);
  await page.locator('.session-feedback-change').click();await page.getByRole('button',{name:'About right',exact:true}).click();
  ...
  await page.getByRole('button',{name:'EDIT',exact:true}).click();await page.locator('.session-feedback-choices').scrollIntoViewIfNeeded();await page.getByRole('button',{name:'Harder than expected',exact:true}).click();await shot(page,`${key}-correction`);await page.getByRole('button',{name:'SAVE CHANGES',exact:true}).click();await page.locator('.completed-workout-detail').waitFor();assert.match(await page.locator('.session-feedback-history').innerText(),/Harder than expected/);
 ...
 for(const scenario of process.env.ROOK_FEEDBACK_BLOCK_ONLY?[]:['ignored','offline','failure','long']){
  ...
  if(scenario==='failure'){await page.getByRole('alert').filter({hasText:'Couldn’t save feedback'}).waitFor();assert.equal((await saved(page)).workouts.at(-1).sessionFeedback,undefined);await shot(page,'320-failure');await page.evaluate(()=>window.restoreFeedbackStorage());}
  await shot(page,`320-${scenario}`,scenario==='long');await page.getByRole('button',{name:'DONE',exact:true}).click();assert.equal(await page.locator('.today-screen').count(),1);await context.close();console.log(`${scenario}: non-blocking completion passed`);
 }
 if(restoreState){const archive=await buildBackupArchive(restoreState,[]),context=await browser.newContext({viewport:{width:390,height:844},serviceWorkers:'block'}),page=await context.newPage();await page.route('**/api/**',r=>r.fulfill({json:{available:false}}));await page.goto('http://127.0.0.1:4173',{waitUntil:'networkidle'});await page.locator('.restore-backup-action').click();await page.getByLabel('Choose ROOK backup file').setInputFiles({name:'feedback.zip',mimeType:'application/zip',buffer:Buffer.from(archive.bytes)});await page.getByRole('button',{name:'RESTORE BACKUP',exact:true}).click();await page.waitForFunction(()=>JSON.parse(localStorage.getItem('lift-v2-state')||'{}').workouts?.some(w=>w.sessionFeedback==='harder'));await shot(page,'390-clean-restore');await context.close();console.log('feedback clean-profile Backup Restore: passed');}
