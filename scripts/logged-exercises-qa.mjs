...
try {for(const [width,appearance,style] of [[390,'light','standard'],[320,'dark','standard'],[390,'light','premium'],[390,'dark','premium']]){
 ...
 assert.equal(await preview.locator('.logged-exercise-row').count(),6);
 ...
 console.log(`PASS ${width} ${style} ${appearance}: six recent, all, search, detail/back, no results, no mutation`);await context.close();
 }
