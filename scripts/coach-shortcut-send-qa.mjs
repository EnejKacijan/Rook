...
try{for(const width of [320,390])for(const style of ['standard','premium'])for(const appearance of ['light','dark'])for(const intent of ['Explain my next workout','Explain my program','Move a workout this week']){
 ...
 await p.route('**/src/aiService.js*',async r=>{const response=await r.fetch();const text=await response.text(),signature=/async coach\(state, message[^)]*\) \{/;assert.ok(signature.test(text));await r.fulfill({response,body:text.replace(signature,match=>match+' window.shortcutCalls=(window.shortcutCalls||0)+1; await new Promise(r=>window.releaseCoach=r);')});});
 await p.goto(process.env.ROOK_QA_URL||'http://127.0.0.1:4173');await p.getByRole('button',{name:'COACH',exact:true}).click();
