...
for(const width of [320,390,430])for(const appearance of ['light','dark'])for(const style of ['standard','premium']){
 ...
 await page.goto('http://127.0.0.1:4173');await openFirstRunLanding(page);await page.getByRole('button',{name:'BUILD MY PLAN',exact:true}).click();
 await page.getByRole('combobox',{name:'Age range'}).click();await page.getByRole('option',{name:'18–29'}).click();
 await page.getByRole('button',{name:'CONTINUE',exact:true}).click();await chooseOnboardingAnswer(page,'Build muscle',3);await chooseOnboardingAnswer(page,/^Beginner/,4);
 ...
 await page.getByRole('button',{name:'CONTINUE',exact:true}).click();await waitForOnboardingStep(page,5);await page.getByRole('button',{name:'Back',exact:true}).click();await waitForOnboardingStep(page,4);
