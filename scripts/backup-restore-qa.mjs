...
await page.locator('.profile-subpage-header').getByText('Data & backup', {exact:true}).waitFor();
await page.screenshot({ path: output("390-profile-data-light-standard.png"), fullPage: true });
await openProfileArea(page, 'data'); await page.getByRole("button", { name: /Back up ROOK/ }).click();
...
await openProfileArea(page, 'data'); await page.getByRole("button", { name: /Restore backup/ }).click();
...
await page.getByRole("button", { name: "PROFILE", exact: true }).click();
await openProfileArea(page, 'data'); await page.getByRole("button", { name: /Restore backup/ }).click();
const beforePreview = JSON.parse(await page.evaluate(() => localStorage.getItem('lift-v2-state')));
...
assert.deepEqual(JSON.parse(await page.evaluate(() => localStorage.getItem('lift-v2-state'))),beforePreview);
assert.equal(beforePreview.profile.name, 'Altered locally');assert.deepEqual(beforePreview.workouts, []);
await page.getByRole('button', {name:'CANCEL',exact:true}).click();
assert.deepEqual(JSON.parse(await page.evaluate(() => localStorage.getItem('lift-v2-state'))),beforePreview);
await page.getByLabel('Choose ROOK backup file').setInputFiles(backupPath);
await page.getByRole('heading',{name:'ROOK backup',exact:true}).waitFor();
assert.deepEqual(JSON.parse(await page.evaluate(() => localStorage.getItem('lift-v2-state'))),beforePreview);
...
// One unavailable local photo never blocks the user's training-data backup.
{
  ...
  await openProfileArea(run.page, 'data'); await run.page.getByRole("button", { name: /Back up ROOK/ }).click();
  ...
  await run.page.getByRole("button", { name: "SAVE BACKUP", exact: true }).click();
...
for (const [appearancePreference, stylePreference, width, label] of [
  ...
  await run.page.getByRole('button',{name:'Close Back up ROOK'}).click();
  await openProfileArea(run.page, 'data'); await run.page.getByRole('button',{name:/Restore backup/}).click();
  await run.page.getByLabel('Choose ROOK backup file').setInputFiles({name:'qa-backup.zip',mimeType:'application/zip',buffer:Buffer.from(bytes)});
  await run.page.getByRole('button',{name:'RESTORE & REPLACE',exact:true}).waitFor();
  await run.page.waitForTimeout(250);
  await run.page.screenshot({path:output(`${width}-restore-preview-${label}.png`)});
