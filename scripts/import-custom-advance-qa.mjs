...
try{for(const width of [320,390]){
 ...
 await openFirstRunLanding(page); await page.locator('.existing-plan-action').click();await page.getByPlaceholder(/Paste your workout notes/).fill('MONDAY - PUSH\nMystery movement alpha 2x8\nBench Press 3x8\nTUESDAY - LEGS\nMystery movement beta 2x10');
 ...
 await page.getByRole('heading',{name:/Mystery movement alpha/i}).waitFor();
 await page.getByRole('button',{name:/KEEP ORIGINAL/}).first().scrollIntoViewIfNeeded();await page.screenshot({path:`${out}/${width}-before.png`});
 await page.getByRole('button',{name:/KEEP ORIGINAL/}).first().click();
 ...
 await page.getByRole('button',{name:/KEEP ORIGINAL/}).first().click();await page.waitForTimeout(300);
