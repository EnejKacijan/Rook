...
await page.getByRole('button', { name: /^Overhead Press/ }).click(); assert.equal(await page.locator('.exercise-heading h1').textContent(), 'Overhead Press'); await page.getByRole('button', { name: 'Complete set 1' }).click(); await page.getByRole('button', { name: 'Finish', exact: true }).click(); await page.getByRole('button', { name: 'FINISH ANYWAY' }).click(); await page.getByRole('button', { name: 'DONE' }).click(); await page.getByRole('button', { name: 'PROGRESS' }).click(); assert.equal(await page.getByText('Overhead Press', { exact: true }).count(), 1, 'custom imported exercise appears in Progress after logging');
