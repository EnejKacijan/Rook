...
// This source-fidelity scenario starts the Monday prescription on that Monday.
const errors = [];
...
await page.goto('http://127.0.0.1:4173', { waitUntil: 'networkidle' });await page.clock.setFixedTime(new Date('2026-09-21T12:00:00'));
...
assert.equal(await page.locator('.plan-editor-exercise.needs-review').count(), 0, 'the current wizard resolves genuine blockers before final review');
...
assert.equal(await page.getByRole('heading',{level:1}).textContent(), 'Barbell Bench Press'); assert.equal(await page.getByRole('textbox', { name: /Weight in kg for set 1/ }).inputValue(), '80'); assert.equal((await page.locator('.up-next').textContent()).includes('Chest Supported Row'), true);
...
await page.locator('.swipe-up-next-body').filter({hasText:'Overhead Press'}).click();await page.locator('.workout-motion-paint').waitFor({state:'detached'}); assert.equal(await page.locator('.exercise-heading h1').textContent(), 'Overhead Press'); await page.getByRole('textbox', { name: /Weight in kg for set 1/ }).fill('30'); await page.getByRole('textbox', { name: /Weight in kg for set 1/ }).press('Enter'); await page.getByRole('button', { name: 'Log set 1' }).click(); await page.getByRole('button', { name: 'Finish', exact: true }).click(); await page.getByRole('button', { name: 'FINISH ANYWAY' }).click(); await page.getByRole('button', { name: 'DONE' }).click(); await page.getByRole('button', { name: 'PROGRESS' }).click(); assert.equal(await page.locator('.progress-screen .logged-exercise-row').filter({hasText:'Overhead Press'}).count(), 1, 'custom imported exercise appears in Progress after logging');
