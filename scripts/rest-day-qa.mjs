...

{
  ...
  await verifyRest(page, 1, '390-rest-next-tomorrow.png'); assert.equal(await page.getByRole('button', { name: 'Train today anyway' }).count(), 1); assert.deepEqual(errors, []); await context.close();
}
{
...
{
  const state = fixture([offsetDay(3), offsetDay(6)]); const { context, page, errors } = await pageFor(state, { width: 390, height: 844 });
  const selectedRest = offsetDay(1); await page.locator(`.week-strip button[aria-label^="${selectedRest} "]`).click();
  await page.getByRole('heading', { name: 'Rest day' }).waitFor(); assert.equal(await page.getByRole('button', { name: 'Train today anyway' }).count(), 0); await page.screenshot({ path: output('390-selected-non-today-rest.png'), fullPage: false }); assert.deepEqual(errors, []); await context.close();
}
{
