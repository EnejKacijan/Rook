...
try {for(const width of [320,390])for(const style of ['standard','premium'])for(const appearance of ['light','dark']) {
  ...
  const page=await context.newPage();page.on('dialog',d=>d.accept());await page.route('**/api/ai/status',r=>r.fulfill({json:{available:false}}));await page.goto('http://127.0.0.1:4173');
  ...
  const remove=async id=>{const row=card(id);if(await row.getAttribute('class').then(c=>!c.includes('is-expanded')))await row.locator('.plan-editor-summary').click();await row.getByRole('button',{name:'Remove exercise',exact:true}).click();};
  await card(day.exercises[0].id).locator('.plan-editor-summary').click();await shot('expanded');await remove(day.exercises[0].id);await sheet.locator('.today-undo').waitFor();assert.equal(await card(day.exercises[0].id).count(),0);await shot('superset-removed');await sheet.getByRole('button',{name:'Undo',exact:true}).click();assert.equal(await card(day.exercises[0].id).count(),1,'Undo restores the exact removed exercise and pairing');await remove(day.exercises[0].id);
  await remove(day.exercises[2].id);await sheet.locator('.today-undo').waitFor();await shot('multiple-removed');assert.equal(await card(day.exercises[0].id).count(),0);assert.equal(await card(day.exercises[2].id).count(),0);
  await sheet.getByRole('button',{name:'Undo',exact:true}).click();await card(day.exercises[2].id).scrollIntoViewIfNeeded();await shot('undo');assert.equal(await card(day.exercises[0].id).count(),0);assert.equal(await card(day.exercises[2].id).count(),1,'the current single Undo restores only the latest removal');
