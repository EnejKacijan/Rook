...
try { for(const [width,appearance,style] of [[320,'dark','standard'],[390,'light','standard'],[320,'dark','premium'],[390,'light','premium'],[390,'dark','premium']]) {
 ...
 await page.goto(process.env.ROOK_QA_URL || 'http://127.0.0.1:4173');await page.locator('.today-screen').waitFor();
 const list=page.locator('.today-completed-workouts');await list.waitFor();
 const additional=state.workouts.filter(w=>mode!=='planned-and-freestyle'||w.id!==state.workouts[0].id).reverse();
 assert.equal(await list.getByText('Completed workouts · 2',{exact:true}).count(),additional.length>1?1:0);
 assert.deepEqual(await list.locator('button').evaluateAll(es=>es.map(e=>e.dataset.workoutId)),additional.map(w=>w.id));
 ...
 await page.reload();await page.locator('.today-completed-workouts').waitFor();assert.equal(await page.locator('.today-completed-workouts button').count(),additional.length);
 if(mode.startsWith('active')){
