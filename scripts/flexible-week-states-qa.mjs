...
for (const width of [320,390,430]) {
  ...
  await page.getByRole('button',{name:'ADJUST WEEK',exact:true}).click(); await page.getByRole('button',{name:/Move a workout Choose/}).click();
...
// Persistence failure: storage writes fail only at the explicit schedule commit.
{
  ...
  await page.getByRole('button',{name:'ADJUST WEEK',exact:true}).click();await page.getByRole('button',{name:/Move a workout Choose/}).click();await page.locator('.flexible-week-sheet .choice-row').first().click();await page.locator('.flexible-week-dates button:not(:disabled)').first().click();
