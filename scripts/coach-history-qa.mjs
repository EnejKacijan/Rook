...
for (const scenario of [{ name: '0-conversations', count: 0 }, { name: '1-conversation', count: 1 }, { name: '5-conversations', count: 5, activeIndex: 2 }, { name: 'long-title', count: 5, longTitle: true }]) {
  ...
  if (!scenario.count) assert.equal(await page.getByRole('heading', { name: 'No conversations yet.' }).count(), 1);
  if (scenario.count) {
...

{
  ...
  await page.screenshot({ path: output('390-18-conversations-scrolled.png'), fullPage: false }); await page.getByRole('button', { name: 'Back to Coach' }).click(); assert.equal(await page.locator('.coach-scroll').evaluate(node => node.scrollTop), coachTopBefore, 'history does not scroll the conversation'); await page.getByRole('button', { name: 'Conversation history' }).click(); assert.ok(Math.abs(await scroller.evaluate(node => node.scrollTop) - savedTop) < 2, 'history scroll position is restored');
