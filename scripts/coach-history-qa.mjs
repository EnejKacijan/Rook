...
async function openHistory(state, width = 390) {
  ...
  await page.goto(process.env.ROOK_QA_URL||'http://127.0.0.1:4173', { waitUntil: 'networkidle' }); await page.getByRole('button', { name: 'COACH', exact: true }).click(); await page.getByRole('button', { name: 'Chat history' }).click(); await page.locator('.coach-history-surface').waitFor(); await page.waitForTimeout(220); return { context, page, errors };
}
...

{
  ...
  await page.screenshot({ path: output('390-18-conversations-scrolled.png'), fullPage: false }); await page.getByRole('button', { name: 'Back to Coach' }).click(); assert.equal(await page.locator('.coach-scroll').evaluate(node => node.scrollTop), coachTopBefore, 'history does not scroll the conversation'); await page.getByRole('button', { name: 'Chat history' }).click(); await page.waitForTimeout(50); assert.ok(Math.abs(await scroller.evaluate(node => node.scrollTop) - savedTop) < 2, 'history scroll position is restored after the opening frame');
  await page.locator('.coach-history-group button').last().click(); assert.equal(await page.locator('.coach-history-surface').isHidden(), true); assert.equal(await page.locator('.coach-history-surface').evaluate(e=>e.inert), true); const stored = await page.evaluate(() => JSON.parse(localStorage.getItem('lift-v2-state'))); assert.notEqual(stored.activeCoachConversationId, 'thread-17'); assert.equal(await page.locator('.coach-input').count(), 0, 'history review has no composer'); assert.deepEqual(stored.conversations.find(entry => entry.actionResult)?.actionResult, appliedResult, 'action result history remains intact'); assert.deepEqual(errors, []); await context.close();
}
