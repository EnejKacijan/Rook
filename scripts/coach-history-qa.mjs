...
async function openHistory(state, width = 390) {
  ...
  await page.goto(process.env.ROOK_QA_URL||'http://127.0.0.1:4173', { waitUntil: 'networkidle' }); await page.getByRole('button', { name: 'COACH', exact: true }).click(); await page.getByRole('button', { name: 'Conversation history' }).click(); await page.locator('.coach-history-surface').waitFor(); await page.waitForTimeout(220); return { context, page, errors };
}
...

{
  ...
  await page.locator('.coach-history-group button').last().click(); assert.equal(await page.locator('.coach-history-surface').isHidden(), true); assert.equal(await page.locator('.coach-history-surface').evaluate(e=>e.inert), true); const stored = await page.evaluate(() => JSON.parse(localStorage.getItem('lift-v2-state'))); assert.equal(stored.activeCoachConversationId, 'thread-17'); assert.deepEqual(stored.conversations.find(entry => entry.actionResult)?.actionResult, appliedResult, 'action result history remains intact'); assert.deepEqual(errors, []); await context.close();
}
